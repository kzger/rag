...
- [x] `docker compose config` 的全部 `ports` 經機器可讀檢查後，集合恰為 loopback 8090/8081/8082/8999；沒有 `0.0.0.0`、`::` 或未指定 host IP 的 publish。
- [x] `ss -ltn` 證明四個允許 port 只綁定 `127.0.0.1`，且 9200、6379、9010/9011、9081、1979、7670/7671/8265、8000–8017 等內部服務 port 未在 host listen。
- [x] 從同一 Compose network 的一次性 test container 可連到每個必要內部 service；從 host published address 無法直接連到內部-only services。
- [x] 每個必要 service 都有 healthcheck 與 restart policy；RAG/ingestor 的 dependency-aware health 只在其必要 dependencies ready 後成功。
- [x] 停止並重建 application containers 後，既有 collection、已 ingest 文件、object data 與 model cache 仍存在，且可再次查詢。
...
- [x] lifecycle 文件不包含 secret，也不建議將 `.env` 或 API keys 複製到 shell history。
...
All agent-executable lifecycle work is complete. The Docker daemon restart is
ready for a privileged operator: `sudo -n systemctl restart docker` was denied
because this session has no sudo credential. An actual second-workstation SSH
tunnel probe also requires the human/operator environment. These two criteria
remain unchecked; the runbook contains the exact verification commands.
